namespace cadpat.bem {
    ...
    class ListagemController {
        ...
        excluir(id: string) {
            ...
            this.$http.delete('/api/v1/bens/' + id)
                .success((response) => {
                    this.$window.alert('Bem excluido com sucesso!');
                    this.listar();
                })
                .error((message) => {
                    this.$window.alert(message);
    ...
    class DetalheController {
        ...
        constructor($http: ng.IHttpService, $routeParams: IRouteParamsServiceBem, $window: ng.IWindowService) {
    ...
    class IncluirController {
        ...
        constructor(private $http: ng.IHttpService, private $location: ng.ILocationService, private $window: ng.IWindowService) {
        ...
        salvar() {
            this.$http.post('/api/v1/bens', this.bem)
                .success((response) => {
                    this.$window.alert('Bem incluido com sucesso');
    ...
    class AlterarController {
        ...
        constructor(private $http: ng.IHttpService, private $location: ng.ILocationService, private $routeParams: IRouteParamsServiceBem, private $window: ng.IWindowService) {
            $http.get('/api/v1/bens/' + $routeParams.id)
                .success((response: IBem) => {
                    this.bem = response;
                })
